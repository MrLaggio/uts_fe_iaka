...
const CustomerList = () => {
  ...
  const fetchCustomers = () => {
    axios.get('http://localhost:4000/pembeli/find')
      .then(response => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (editingCustomer) {
      axios.put(`http://localhost:4000/updatePembeli/${editingCustomer.id}`, formData)
        .then(() => {
          fetchCustomers();
          handleCloseModal();
        })
      ...
      axios.post('http://localhost:4000/createPembeli', formData)
        .then(() => {
          fetchCustomers();
          handleCloseModal();
        })
  ...
  const handleDelete = (id) => {
    if (window.confirm('Are you sure you want to delete this customer?')) {
      axios.delete(`http://localhost:4000/deletePembeli/${id}`)
        .then(() => {
          fetchCustomers();
        })
            ...
            <tbody className="divide-y divide-gray-200">
              {customers.map(customer => (
                <tr key={customer.id}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">{customer.no_telp}</td>
          ...
          <div className="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
            <div className="mt-3 text-center">
              ...
              <form className="mt-2 text-left" onSubmit={handleSubmit}>
                ...
                <div className="mb-4">
                  <label className="block text-gray-700 text-sm font-bold mb-2" htmlFor="no_telp">
                    Phone
                  </label>
                  <input
                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
